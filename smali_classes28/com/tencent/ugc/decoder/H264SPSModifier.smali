.class public Lcom/tencent/ugc/decoder/H264SPSModifier;
.super Ljava/lang/Object;
.source "SourceFile"


# static fields
.field private static final DEBUG:Z = false

.field private static final TAG:Ljava/lang/String; = "H264SPSModifier"


# instance fields
.field private mReceiveFirstVUI:Z


# direct methods
.method public constructor <init>()V
    .locals 1

    .line 1
    .line 2
    .line 3
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 4
    const/4 v0, 0x0

    .line 5
    .line 6
    iput-boolean v0, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 7
    return-void
    .line 8
    .line 9
    .line 10
    .line 11
    .line 12
    .line 13
    .line 14
    .line 15
    .line 16
    .line 17
    .line 18
.end method

.method private deEmulationPrevention([B)[B
    .locals 9

    .line 1
    array-length v0, p1

    .line 2
    .line 3
    new-array v0, v0, [B

    .line 4
    const/4 v1, 0x0

    .line 5
    const/4 v2, 0x0

    .line 6
    const/4 v3, 0x0

    .line 7
    :goto_0
    array-length v4, p1

    .line 8
    .line 9
    if-ge v2, v4, :cond_1

    .line 10
    array-length v4, p1

    .line 11
    const/4 v5, 0x3

    .line 12
    sub-int/2addr v4, v5

    .line 13
    .line 14
    if-ge v2, v4, :cond_0

    .line 15
    .line 16
    aget-byte v4, p1, v2

    .line 17
    .line 18
    if-nez v4, :cond_0

    .line 19
    .line 20
    add-int/lit8 v6, v2, 0x1

    .line 21
    .line 22
    aget-byte v7, p1, v6

    .line 23
    .line 24
    if-nez v7, :cond_0

    .line 25
    .line 26
    add-int/lit8 v7, v2, 0x2

    .line 27
    .line 28
    aget-byte v7, p1, v7

    .line 29
    .line 30
    if-ne v7, v5, :cond_0

    .line 31
    .line 32
    add-int/lit8 v7, v2, 0x3

    .line 33
    .line 34
    aget-byte v8, p1, v7

    .line 35
    .line 36
    if-gt v8, v5, :cond_0

    .line 37
    .line 38
    add-int/lit8 v2, v3, 0x1

    .line 39
    .line 40
    aput-byte v4, v0, v3

    .line 41
    .line 42
    add-int/lit8 v3, v2, 0x1

    .line 43
    .line 44
    aget-byte v4, p1, v6

    .line 45
    .line 46
    aput-byte v4, v0, v2

    .line 47
    move v2, v7

    .line 48
    goto :goto_0

    .line 49
    .line 50
    :cond_0
    add-int/lit8 v4, v3, 0x1

    .line 51
    .line 52
    aget-byte v5, p1, v2

    .line 53
    .line 54
    aput-byte v5, v0, v3

    .line 55
    .line 56
    add-int/lit8 v2, v2, 0x1

    .line 57
    move v3, v4

    .line 58
    goto :goto_0

    .line 59
    :cond_1
    array-length p1, p1

    .line 60
    .line 61
    if-eq v3, p1, :cond_2

    .line 62
    .line 63
    new-array p1, v3, [B

    .line 64
    .line 65
    .line 66
    invoke-static {v0, v1, p1, v1, v3}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    .line 67
    return-object p1

    .line 68
    :cond_2
    const/4 p1, 0x0

    .line 69
    return-object p1
    .line 70
    .line 71
    .line 72
    .line 73
.end method

.method private emulationPrevention([B)[B
    .locals 9

    .line 1
    array-length v0, p1

    .line 2
    const/4 v1, 0x3

    .line 3
    .line 4
    mul-int/lit8 v0, v0, 0x3

    .line 5
    .line 6
    div-int/lit8 v0, v0, 0x2

    .line 7
    .line 8
    new-array v0, v0, [B

    .line 9
    const/4 v2, 0x0

    .line 10
    const/4 v3, 0x0

    .line 11
    const/4 v4, 0x0

    .line 12
    :goto_0
    array-length v5, p1

    .line 13
    .line 14
    if-ge v3, v5, :cond_1

    .line 15
    array-length v5, p1

    .line 16
    .line 17
    add-int/lit8 v5, v5, -0x2

    .line 18
    .line 19
    if-ge v3, v5, :cond_0

    .line 20
    .line 21
    aget-byte v5, p1, v3

    .line 22
    .line 23
    if-nez v5, :cond_0

    .line 24
    .line 25
    add-int/lit8 v6, v3, 0x1

    .line 26
    .line 27
    aget-byte v7, p1, v6

    .line 28
    .line 29
    if-nez v7, :cond_0

    .line 30
    .line 31
    add-int/lit8 v7, v3, 0x2

    .line 32
    .line 33
    aget-byte v8, p1, v7

    .line 34
    .line 35
    if-gt v8, v1, :cond_0

    .line 36
    .line 37
    add-int/lit8 v3, v4, 0x1

    .line 38
    .line 39
    aput-byte v5, v0, v4

    .line 40
    .line 41
    add-int/lit8 v4, v3, 0x1

    .line 42
    .line 43
    aget-byte v5, p1, v6

    .line 44
    .line 45
    aput-byte v5, v0, v3

    .line 46
    .line 47
    add-int/lit8 v3, v4, 0x1

    .line 48
    .line 49
    aput-byte v1, v0, v4

    .line 50
    move v4, v3

    .line 51
    move v3, v7

    .line 52
    goto :goto_0

    .line 53
    .line 54
    :cond_0
    add-int/lit8 v5, v4, 0x1

    .line 55
    .line 56
    aget-byte v6, p1, v3

    .line 57
    .line 58
    aput-byte v6, v0, v4

    .line 59
    .line 60
    add-int/lit8 v3, v3, 0x1

    .line 61
    move v4, v5

    .line 62
    goto :goto_0

    .line 63
    :cond_1
    array-length v1, p1

    .line 64
    .line 65
    if-eq v4, v1, :cond_2

    .line 66
    .line 67
    new-array p1, v4, [B

    .line 68
    .line 69
    .line 70
    invoke-static {v0, v2, p1, v2, v4}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    .line 71
    :cond_2
    return-object p1
    .line 72
    .line 73
.end method

.method private modifyVUIParameters(Lcom/tencent/ugc/decoder/H264RbspRewriter;)Z
    .locals 6
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 1
    .line 2
    const-string/jumbo v0, "VUI: aspect_ratio_info_present_flag"

    .line 3
    .line 4
    .line 5
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 6
    move-result v0

    .line 7
    .line 8
    const/16 v1, 0x8

    .line 9
    .line 10
    if-eqz v0, :cond_0

    .line 11
    .line 12
    const-string/jumbo v0, "VUI: aspect_ratio"

    .line 13
    .line 14
    .line 15
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readNBit(ILjava/lang/String;)J

    .line 16
    move-result-wide v2

    .line 17
    long-to-int v0, v2

    .line 18
    .line 19
    const/16 v2, 0xff

    .line 20
    .line 21
    if-ne v0, v2, :cond_0

    .line 22
    .line 23
    const-string/jumbo v0, "VUI: sar_width"

    .line 24
    .line 25
    const/16 v2, 0x10

    .line 26
    .line 27
    .line 28
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 29
    .line 30
    const-string/jumbo v0, "VUI: sar_height"

    .line 31
    .line 32
    .line 33
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 34
    .line 35
    :cond_0
    const-string/jumbo v0, "VUI: overscan_info_present_flag"

    .line 36
    .line 37
    .line 38
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 39
    move-result v0

    .line 40
    const/4 v2, 0x1

    .line 41
    .line 42
    if-eqz v0, :cond_1

    .line 43
    .line 44
    const-string/jumbo v0, "VUI: overscan_appropriate_flag"

    .line 45
    .line 46
    .line 47
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 48
    .line 49
    :cond_1
    const-string/jumbo v0, "VUI: video_signal_type_present_flag"

    .line 50
    .line 51
    .line 52
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 53
    move-result v0

    .line 54
    .line 55
    if-eqz v0, :cond_2

    .line 56
    const/4 v0, 0x3

    .line 57
    .line 58
    const-string/jumbo v3, "VUI: video_format"

    .line 59
    .line 60
    .line 61
    invoke-virtual {p1, v0, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 62
    .line 63
    const-string/jumbo v0, "VUI: video_full_range_flag"

    .line 64
    .line 65
    .line 66
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 67
    .line 68
    const-string/jumbo v0, "VUI: colour_description_present_flag"

    .line 69
    .line 70
    .line 71
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 72
    move-result v0

    .line 73
    .line 74
    if-eqz v0, :cond_2

    .line 75
    .line 76
    const-string/jumbo v0, "VUI: colour_primaries"

    .line 77
    .line 78
    .line 79
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 80
    .line 81
    const-string/jumbo v0, "VUI: transfer_characteristics"

    .line 82
    .line 83
    .line 84
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 85
    .line 86
    const-string/jumbo v0, "VUI: matrix_coefficients"

    .line 87
    .line 88
    .line 89
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 90
    .line 91
    :cond_2
    const-string/jumbo v0, "VUI: chroma_loc_info_present_flag"

    .line 92
    .line 93
    .line 94
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 95
    move-result v0

    .line 96
    .line 97
    if-eqz v0, :cond_3

    .line 98
    .line 99
    const-string/jumbo v0, "VUI chroma_sample_loc_type_top_field"

    .line 100
    .line 101
    .line 102
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 103
    .line 104
    const-string/jumbo v0, "VUI chroma_sample_loc_type_bottom_field"

    .line 105
    .line 106
    .line 107
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 108
    .line 109
    :cond_3
    const-string/jumbo v0, "VUI: timing_info_present_flag"

    .line 110
    .line 111
    .line 112
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 113
    move-result v0

    .line 114
    .line 115
    if-eqz v0, :cond_4

    .line 116
    .line 117
    const-string/jumbo v0, "VUI: num_units_in_tick"

    .line 118
    .line 119
    const/16 v1, 0x20

    .line 120
    .line 121
    .line 122
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 123
    .line 124
    const-string/jumbo v0, "VUI: time_scale"

    .line 125
    .line 126
    .line 127
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 128
    .line 129
    const-string/jumbo v0, "VUI: fixed_frame_rate_flag"

    .line 130
    .line 131
    .line 132
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 133
    .line 134
    :cond_4
    const-string/jumbo v0, "VUI: nal_hrd_parameters_present_flag"

    .line 135
    .line 136
    .line 137
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 138
    move-result v0

    .line 139
    .line 140
    if-eqz v0, :cond_5

    .line 141
    .line 142
    .line 143
    invoke-direct {p0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->readHRDParameters(Lcom/tencent/ugc/decoder/H264RbspRewriter;)V

    .line 144
    .line 145
    :cond_5
    const-string/jumbo v1, "VUI: vcl_hrd_parameters_present_flag"

    .line 146
    .line 147
    .line 148
    invoke-virtual {p1, v1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 149
    move-result v1

    .line 150
    .line 151
    if-eqz v1, :cond_6

    .line 152
    .line 153
    .line 154
    invoke-direct {p0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->readHRDParameters(Lcom/tencent/ugc/decoder/H264RbspRewriter;)V

    .line 155
    .line 156
    :cond_6
    if-nez v0, :cond_7

    .line 157
    .line 158
    if-eqz v1, :cond_8

    .line 159
    .line 160
    :cond_7
    const-string/jumbo v0, "VUI: low_delay_hrd_flag"

    .line 161
    .line 162
    .line 163
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 164
    .line 165
    :cond_8
    const-string/jumbo v0, "VUI: pic_struct_present_flag"

    .line 166
    .line 167
    .line 168
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 169
    .line 170
    const-string/jumbo v0, "VUI: bitstream_restriction_flag"

    .line 171
    .line 172
    .line 173
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->testBool(Ljava/lang/String;)Z

    .line 174
    move-result v0

    .line 175
    .line 176
    const-string/jumbo v1, "H264SPSModifier"

    .line 177
    .line 178
    const-string/jumbo v3, "VUI: motion_vectors_over_pic_boundaries_flag"

    .line 179
    .line 180
    const-string/jumbo v4, "VUI: set bitstream_restriction_flag"

    .line 181
    const/4 v5, 0x0

    .line 182
    .line 183
    if-eqz v0, :cond_a

    .line 184
    .line 185
    .line 186
    invoke-virtual {p1, v2, v4}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    .line 187
    .line 188
    .line 189
    invoke-virtual {p1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    .line 190
    .line 191
    const-string/jumbo v0, "VUI max_bytes_per_pic_denom"

    .line 192
    .line 193
    .line 194
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 195
    .line 196
    const-string/jumbo v0, "VUI max_bits_per_mb_denom"

    .line 197
    .line 198
    .line 199
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 200
    .line 201
    const-string/jumbo v0, "VUI log2_max_mv_length_horizontal"

    .line 202
    .line 203
    .line 204
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 205
    .line 206
    const-string/jumbo v0, "VUI log2_max_mv_length_vertical"

    .line 207
    .line 208
    .line 209
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 210
    .line 211
    const-string/jumbo v0, "VUI num_reorder_frames"

    .line 212
    .line 213
    .line 214
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 215
    .line 216
    iget-boolean v0, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 217
    .line 218
    if-nez v0, :cond_9

    .line 219
    .line 220
    .line 221
    invoke-virtual {p1, v5}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Z)I

    .line 222
    move-result p1

    .line 223
    .line 224
    const-string/jumbo v0, "decode: do not add max_dec_frame_buffering when it is "

    .line 225
    .line 226
    .line 227
    invoke-static {p1}, Ljava/lang/String;->valueOf(I)Ljava/lang/String;

    .line 228
    move-result-object p1

    .line 229
    .line 230
    .line 231
    invoke-virtual {v0, p1}, Ljava/lang/String;->concat(Ljava/lang/String;)Ljava/lang/String;

    .line 232
    move-result-object p1

    .line 233
    .line 234
    .line 235
    invoke-static {v1, p1}, Lcom/tencent/liteav/base/util/LiteavLog;->w(Ljava/lang/String;Ljava/lang/String;)V

    .line 236
    .line 237
    iput-boolean v2, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 238
    :cond_9
    return v5

    .line 239
    .line 240
    .line 241
    :cond_a
    invoke-virtual {p1, v2, v4}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    .line 242
    .line 243
    .line 244
    invoke-virtual {p1, v2, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    .line 245
    .line 246
    const-string/jumbo v0, "VUI: max_bytes_per_pic_denom"

    .line 247
    .line 248
    .line 249
    invoke-virtual {p1, v5, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 250
    .line 251
    const-string/jumbo v0, "VUI: max_bits_per_mb_denom"

    .line 252
    .line 253
    .line 254
    invoke-virtual {p1, v5, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 255
    .line 256
    const-string/jumbo v0, "VUI: log2_max_mv_length_horizontal"

    .line 257
    .line 258
    const/16 v3, 0xa

    .line 259
    .line 260
    .line 261
    invoke-virtual {p1, v3, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 262
    .line 263
    const-string/jumbo v0, "VUI: log2_max_mv_length_vertical"

    .line 264
    .line 265
    .line 266
    invoke-virtual {p1, v3, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 267
    .line 268
    const-string/jumbo v0, "VUI: num_reorder_frames"

    .line 269
    .line 270
    .line 271
    invoke-virtual {p1, v5, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 272
    .line 273
    const-string/jumbo v0, "VUI: max_dec_frame_buffering"

    .line 274
    .line 275
    .line 276
    invoke-virtual {p1, v2, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 277
    .line 278
    iget-boolean p1, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 279
    .line 280
    if-nez p1, :cond_b

    .line 281
    .line 282
    const-string/jumbo p1, "decode: add max_dec_frame_buffering 1 when it is no exist"

    .line 283
    .line 284
    .line 285
    invoke-static {v1, p1}, Lcom/tencent/liteav/base/util/LiteavLog;->w(Ljava/lang/String;Ljava/lang/String;)V

    .line 286
    .line 287
    iput-boolean v2, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 288
    :cond_b
    return v2
    .line 289
    .line 290
    .line 291
    .line 292
    .line 293
    .line 294
    .line 295
    .line 296
    .line 297
    .line 298
    .line 299
    .line 300
    .line 301
    .line 302
    .line 303
    .line 304
    .line 305
    .line 306
    .line 307
    .line 308
    .line 309
    .line 310
    .line 311
    .line 312
    .line 313
    .line 314
    .line 315
    .line 316
    .line 317
    .line 318
    .line 319
    .line 320
    .line 321
    .line 322
    .line 323
    .line 324
    .line 325
    .line 326
    .line 327
    .line 328
    .line 329
    .line 330
    .line 331
    .line 332
    .line 333
    .line 334
    .line 335
    .line 336
    .line 337
    .line 338
    .line 339
    .line 340
    .line 341
    .line 342
    .line 343
    .line 344
    .line 345
    .line 346
    .line 347
    .line 348
    .line 349
    .line 350
    .line 351
    .line 352
    .line 353
    .line 354
    .line 355
    .line 356
    .line 357
    .line 358
    .line 359
    .line 360
    .line 361
    .line 362
    .line 363
    .line 364
    .line 365
    .line 366
    .line 367
    .line 368
    .line 369
    .line 370
    .line 371
    .line 372
    .line 373
    .line 374
    .line 375
    .line 376
    .line 377
    .line 378
    .line 379
    .line 380
    .line 381
    .line 382
    .line 383
    .line 384
    .line 385
    .line 386
    .line 387
    .line 388
    .line 389
    .line 390
    .line 391
    .line 392
    .line 393
    .line 394
    .line 395
    .line 396
    .line 397
    .line 398
    .line 399
    .line 400
    .line 401
    .line 402
    .line 403
    .line 404
    .line 405
    .line 406
    .line 407
    .line 408
    .line 409
    .line 410
    .line 411
    .line 412
    .line 413
    .line 414
    .line 415
    .line 416
    .line 417
    .line 418
    .line 419
    .line 420
    .line 421
    .line 422
    .line 423
    .line 424
    .line 425
    .line 426
    .line 427
    .line 428
    .line 429
    .line 430
    .line 431
    .line 432
    .line 433
    .line 434
    .line 435
    .line 436
    .line 437
    .line 438
    .line 439
    .line 440
    .line 441
    .line 442
    .line 443
    .line 444
    .line 445
    .line 446
    .line 447
    .line 448
    .line 449
    .line 450
    .line 451
    .line 452
    .line 453
    .line 454
    .line 455
    .line 456
    .line 457
    .line 458
    .line 459
    .line 460
    .line 461
    .line 462
    .line 463
    .line 464
    .line 465
    .line 466
    .line 467
    .line 468
    .line 469
    .line 470
    .line 471
    .line 472
    .line 473
    .line 474
    .line 475
    .line 476
    .line 477
    .line 478
    .line 479
    .line 480
    .line 481
    .line 482
    .line 483
    .line 484
    .line 485
    .line 486
    .line 487
    .line 488
    .line 489
    .line 490
    .line 491
    .line 492
    .line 493
    .line 494
    .line 495
    .line 496
    .line 497
    .line 498
    .line 499
    .line 500
    .line 501
    .line 502
    .line 503
    .line 504
    .line 505
    .line 506
    .line 507
    .line 508
    .line 509
    .line 510
    .line 511
    .line 512
    .line 513
    .line 514
    .line 515
    .line 516
    .line 517
    .line 518
    .line 519
    .line 520
    .line 521
    .line 522
    .line 523
    .line 524
    .line 525
    .line 526
    .line 527
    .line 528
    .line 529
    .line 530
    .line 531
    .line 532
    .line 533
    .line 534
    .line 535
    .line 536
    .line 537
    .line 538
    .line 539
    .line 540
    .line 541
    .line 542
    .line 543
    .line 544
    .line 545
    .line 546
    .line 547
    .line 548
    .line 549
    .line 550
    .line 551
    .line 552
    .line 553
    .line 554
    .line 555
    .line 556
    .line 557
    .line 558
    .line 559
    .line 560
    .line 561
    .line 562
    .line 563
    .line 564
    .line 565
    .line 566
    .line 567
    .line 568
    .line 569
    .line 570
    .line 571
    .line 572
    .line 573
    .line 574
    .line 575
    .line 576
    .line 577
    .line 578
    .line 579
    .line 580
    .line 581
    .line 582
    .line 583
    .line 584
    .line 585
    .line 586
    .line 587
    .line 588
    .line 589
    .line 590
    .line 591
    .line 592
    .line 593
    .line 594
    .line 595
    .line 596
    .line 597
    .line 598
    .line 599
    .line 600
    .line 601
    .line 602
    .line 603
    .line 604
    .line 605
    .line 606
    .line 607
    .line 608
    .line 609
    .line 610
    .line 611
    .line 612
    .line 613
    .line 614
    .line 615
    .line 616
    .line 617
    .line 618
    .line 619
    .line 620
    .line 621
    .line 622
    .line 623
    .line 624
    .line 625
    .line 626
    .line 627
    .line 628
    .line 629
    .line 630
    .line 631
    .line 632
    .line 633
    .line 634
    .line 635
    .line 636
    .line 637
    .line 638
    .line 639
    .line 640
    .line 641
    .line 642
    .line 643
    .line 644
    .line 645
    .line 646
    .line 647
    .line 648
    .line 649
    .line 650
    .line 651
    .line 652
    .line 653
    .line 654
    .line 655
    .line 656
    .line 657
    .line 658
    .line 659
    .line 660
    .line 661
    .line 662
    .line 663
    .line 664
    .line 665
    .line 666
    .line 667
    .line 668
    .line 669
    .line 670
    .line 671
    .line 672
    .line 673
    .line 674
    .line 675
    .line 676
    .line 677
    .line 678
    .line 679
    .line 680
    .line 681
    .line 682
    .line 683
    .line 684
    .line 685
    .line 686
    .line 687
    .line 688
    .line 689
    .line 690
    .line 691
    .line 692
    .line 693
    .line 694
    .line 695
    .line 696
    .line 697
    .line 698
    .line 699
    .line 700
    .line 701
    .line 702
    .line 703
    .line 704
    .line 705
    .line 706
    .line 707
    .line 708
    .line 709
    .line 710
    .line 711
    .line 712
    .line 713
    .line 714
    .line 715
    .line 716
    .line 717
    .line 718
    .line 719
    .line 720
    .line 721
    .line 722
    .line 723
    .line 724
    .line 725
    .line 726
    .line 727
    .line 728
    .line 729
    .line 730
    .line 731
    .line 732
    .line 733
    .line 734
    .line 735
    .line 736
    .line 737
    .line 738
    .line 739
    .line 740
    .line 741
    .line 742
    .line 743
    .line 744
    .line 745
    .line 746
    .line 747
    .line 748
    .line 749
    .line 750
    .line 751
    .line 752
    .line 753
    .line 754
    .line 755
    .line 756
    .line 757
    .line 758
    .line 759
    .line 760
    .line 761
    .line 762
    .line 763
    .line 764
    .line 765
    .line 766
    .line 767
    .line 768
    .line 769
    .line 770
    .line 771
    .line 772
    .line 773
    .line 774
    .line 775
    .line 776
    .line 777
    .line 778
    .line 779
    .line 780
    .line 781
    .line 782
    .line 783
    .line 784
    .line 785
    .line 786
    .line 787
    .line 788
    .line 789
    .line 790
    .line 791
    .line 792
    .line 793
    .line 794
    .line 795
    .line 796
    .line 797
    .line 798
    .line 799
    .line 800
    .line 801
    .line 802
    .line 803
    .line 804
    .line 805
    .line 806
    .line 807
    .line 808
    .line 809
    .line 810
    .line 811
    .line 812
    .line 813
    .line 814
    .line 815
    .line 816
    .line 817
    .line 818
    .line 819
    .line 820
    .line 821
    .line 822
    .line 823
    .line 824
    .line 825
    .line 826
    .line 827
    .line 828
    .line 829
    .line 830
    .line 831
    .line 832
    .line 833
    .line 834
    .line 835
    .line 836
    .line 837
    .line 838
    .line 839
    .line 840
    .line 841
    .line 842
    .line 843
    .line 844
    .line 845
    .line 846
    .line 847
    .line 848
    .line 849
    .line 850
    .line 851
    .line 852
    .line 853
    .line 854
    .line 855
    .line 856
    .line 857
    .line 858
    .line 859
    .line 860
    .line 861
    .line 862
    .line 863
    .line 864
    .line 865
    .line 866
    .line 867
    .line 868
    .line 869
    .line 870
    .line 871
    .line 872
    .line 873
    .line 874
    .line 875
    .line 876
    .line 877
    .line 878
    .line 879
    .line 880
    .line 881
    .line 882
    .line 883
    .line 884
    .line 885
    .line 886
    .line 887
    .line 888
    .line 889
    .line 890
    .line 891
    .line 892
    .line 893
    .line 894
    .line 895
    .line 896
    .line 897
    .line 898
    .line 899
    .line 900
    .line 901
    .line 902
    .line 903
    .line 904
    .line 905
    .line 906
    .line 907
    .line 908
    .line 909
    .line 910
    .line 911
    .line 912
    .line 913
    .line 914
    .line 915
    .line 916
    .line 917
    .line 918
    .line 919
    .line 920
    .line 921
    .line 922
    .line 923
    .line 924
    .line 925
    .line 926
    .line 927
    .line 928
    .line 929
    .line 930
    .line 931
    .line 932
    .line 933
    .line 934
    .line 935
    .line 936
    .line 937
    .line 938
    .line 939
    .line 940
    .line 941
    .line 942
    .line 943
    .line 944
    .line 945
    .line 946
    .line 947
    .line 948
    .line 949
    .line 950
    .line 951
    .line 952
    .line 953
    .line 954
    .line 955
    .line 956
    .line 957
    .line 958
    .line 959
    .line 960
    .line 961
    .line 962
    .line 963
    .line 964
    .line 965
    .line 966
    .line 967
    .line 968
    .line 969
    .line 970
    .line 971
    .line 972
    .line 973
    .line 974
    .line 975
    .line 976
    .line 977
    .line 978
    .line 979
    .line 980
    .line 981
    .line 982
    .line 983
    .line 984
    .line 985
    .line 986
    .line 987
    .line 988
    .line 989
    .line 990
    .line 991
    .line 992
    .line 993
    .line 994
    .line 995
    .line 996
    .line 997
    .line 998
    .line 999
    .line 1000
    .line 1001
    .line 1002
    .line 1003
    .line 1004
    .line 1005
    .line 1006
    .line 1007
    .line 1008
    .line 1009
    .line 1010
    .line 1011
    .line 1012
    .line 1013
    .line 1014
    .line 1015
    .line 1016
    .line 1017
    .line 1018
    .line 1019
    .line 1020
    .line 1021
    .line 1022
    .line 1023
    .line 1024
    .line 1025
    .line 1026
    .line 1027
    .line 1028
    .line 1029
    .line 1030
    .line 1031
    .line 1032
    .line 1033
    .line 1034
    .line 1035
    .line 1036
    .line 1037
    .line 1038
    .line 1039
    .line 1040
    .line 1041
    .line 1042
    .line 1043
    .line 1044
    .line 1045
    .line 1046
    .line 1047
    .line 1048
    .line 1049
    .line 1050
    .line 1051
    .line 1052
    .line 1053
    .line 1054
    .line 1055
    .line 1056
    .line 1057
    .line 1058
    .line 1059
    .line 1060
    .line 1061
    .line 1062
    .line 1063
    .line 1064
    .line 1065
    .line 1066
    .line 1067
    .line 1068
    .line 1069
    .line 1070
    .line 1071
    .line 1072
    .line 1073
    .line 1074
    .line 1075
    .line 1076
    .line 1077
    .line 1078
    .line 1079
    .line 1080
    .line 1081
    .line 1082
    .line 1083
    .line 1084
    .line 1085
    .line 1086
    .line 1087
    .line 1088
    .line 1089
    .line 1090
    .line 1091
    .line 1092
    .line 1093
    .line 1094
    .line 1095
    .line 1096
    .line 1097
    .line 1098
    .line 1099
    .line 1100
    .line 1101
    .line 1102
    .line 1103
    .line 1104
    .line 1105
    .line 1106
    .line 1107
    .line 1108
    .line 1109
    .line 1110
    .line 1111
    .line 1112
    .line 1113
    .line 1114
    .line 1115
    .line 1116
    .line 1117
    .line 1118
    .line 1119
    .line 1120
    .line 1121
    .line 1122
    .line 1123
    .line 1124
    .line 1125
    .line 1126
    .line 1127
    .line 1128
    .line 1129
    .line 1130
    .line 1131
    .line 1132
    .line 1133
    .line 1134
    .line 1135
    .line 1136
    .line 1137
    .line 1138
    .line 1139
    .line 1140
    .line 1141
    .line 1142
    .line 1143
    .line 1144
    .line 1145
    .line 1146
    .line 1147
    .line 1148
    .line 1149
    .line 1150
    .line 1151
    .line 1152
    .line 1153
    .line 1154
    .line 1155
    .line 1156
    .line 1157
    .line 1158
    .line 1159
    .line 1160
    .line 1161
    .line 1162
    .line 1163
    .line 1164
    .line 1165
    .line 1166
    .line 1167
    .line 1168
    .line 1169
    .line 1170
    .line 1171
    .line 1172
    .line 1173
    .line 1174
    .line 1175
    .line 1176
    .line 1177
    .line 1178
    .line 1179
    .line 1180
    .line 1181
    .line 1182
    .line 1183
    .line 1184
    .line 1185
    .line 1186
    .line 1187
    .line 1188
    .line 1189
    .line 1190
    .line 1191
    .line 1192
    .line 1193
    .line 1194
    .line 1195
    .line 1196
    .line 1197
    .line 1198
    .line 1199
    .line 1200
    .line 1201
    .line 1202
    .line 1203
    .line 1204
    .line 1205
    .line 1206
    .line 1207
    .line 1208
    .line 1209
    .line 1210
    .line 1211
    .line 1212
    .line 1213
    .line 1214
    .line 1215
    .line 1216
    .line 1217
    .line 1218
    .line 1219
    .line 1220
    .line 1221
    .line 1222
    .line 1223
    .line 1224
    .line 1225
    .line 1226
    .line 1227
    .line 1228
    .line 1229
    .line 1230
    .line 1231
    .line 1232
    .line 1233
    .line 1234
    .line 1235
    .line 1236
    .line 1237
    .line 1238
    .line 1239
    .line 1240
    .line 1241
    .line 1242
    .line 1243
    .line 1244
    .line 1245
    .line 1246
    .line 1247
    .line 1248
    .line 1249
    .line 1250
    .line 1251
    .line 1252
    .line 1253
    .line 1254
    .line 1255
    .line 1256
    .line 1257
    .line 1258
    .line 1259
    .line 1260
    .line 1261
    .line 1262
    .line 1263
    .line 1264
    .line 1265
    .line 1266
    .line 1267
    .line 1268
    .line 1269
    .line 1270
    .line 1271
    .line 1272
    .line 1273
    .line 1274
    .line 1275
    .line 1276
    .line 1277
    .line 1278
    .line 1279
    .line 1280
    .line 1281
    .line 1282
    .line 1283
    .line 1284
    .line 1285
    .line 1286
    .line 1287
    .line 1288
    .line 1289
    .line 1290
    .line 1291
    .line 1292
    .line 1293
    .line 1294
    .line 1295
    .line 1296
    .line 1297
    .line 1298
    .line 1299
    .line 1300
    .line 1301
    .line 1302
    .line 1303
    .line 1304
    .line 1305
    .line 1306
    .line 1307
    .line 1308
    .line 1309
    .line 1310
    .line 1311
    .line 1312
    .line 1313
    .line 1314
    .line 1315
    .line 1316
    .line 1317
    .line 1318
    .line 1319
    .line 1320
    .line 1321
    .line 1322
    .line 1323
    .line 1324
    .line 1325
    .line 1326
    .line 1327
    .line 1328
    .line 1329
    .line 1330
    .line 1331
    .line 1332
    .line 1333
    .line 1334
    .line 1335
    .line 1336
    .line 1337
    .line 1338
    .line 1339
    .line 1340
    .line 1341
    .line 1342
    .line 1343
    .line 1344
    .line 1345
    .line 1346
    .line 1347
    .line 1348
    .line 1349
    .line 1350
    .line 1351
    .line 1352
    .line 1353
    .line 1354
    .line 1355
    .line 1356
    .line 1357
    .line 1358
    .line 1359
    .line 1360
    .line 1361
    .line 1362
    .line 1363
    .line 1364
    .line 1365
    .line 1366
    .line 1367
    .line 1368
    .line 1369
    .line 1370
    .line 1371
    .line 1372
    .line 1373
    .line 1374
    .line 1375
    .line 1376
    .line 1377
    .line 1378
    .line 1379
    .line 1380
    .line 1381
    .line 1382
    .line 1383
    .line 1384
    .line 1385
    .line 1386
    .line 1387
    .line 1388
    .line 1389
    .line 1390
    .line 1391
    .line 1392
    .line 1393
    .line 1394
    .line 1395
    .line 1396
    .line 1397
    .line 1398
    .line 1399
    .line 1400
    .line 1401
    .line 1402
    .line 1403
    .line 1404
    .line 1405
    .line 1406
    .line 1407
    .line 1408
    .line 1409
    .line 1410
    .line 1411
    .line 1412
    .line 1413
    .line 1414
    .line 1415
    .line 1416
    .line 1417
    .line 1418
    .line 1419
    .line 1420
    .line 1421
    .line 1422
    .line 1423
    .line 1424
    .line 1425
    .line 1426
    .line 1427
    .line 1428
    .line 1429
    .line 1430
    .line 1431
    .line 1432
    .line 1433
    .line 1434
    .line 1435
    .line 1436
    .line 1437
    .line 1438
    .line 1439
    .line 1440
    .line 1441
    .line 1442
    .line 1443
    .line 1444
    .line 1445
    .line 1446
    .line 1447
    .line 1448
    .line 1449
    .line 1450
    .line 1451
    .line 1452
    .line 1453
    .line 1454
    .line 1455
    .line 1456
    .line 1457
    .line 1458
    .line 1459
    .line 1460
    .line 1461
    .line 1462
    .line 1463
    .line 1464
    .line 1465
    .line 1466
    .line 1467
    .line 1468
    .line 1469
    .line 1470
    .line 1471
    .line 1472
    .line 1473
    .line 1474
    .line 1475
    .line 1476
    .line 1477
    .line 1478
    .line 1479
    .line 1480
    .line 1481
    .line 1482
    .line 1483
    .line 1484
    .line 1485
    .line 1486
    .line 1487
    .line 1488
    .line 1489
    .line 1490
    .line 1491
    .line 1492
    .line 1493
    .line 1494
    .line 1495
    .line 1496
    .line 1497
    .line 1498
    .line 1499
    .line 1500
    .line 1501
    .line 1502
    .line 1503
    .line 1504
    .line 1505
    .line 1506
    .line 1507
    .line 1508
    .line 1509
    .line 1510
    .line 1511
    .line 1512
    .line 1513
    .line 1514
    .line 1515
    .line 1516
    .line 1517
    .line 1518
    .line 1519
    .line 1520
    .line 1521
    .line 1522
    .line 1523
    .line 1524
    .line 1525
    .line 1526
    .line 1527
    .line 1528
    .line 1529
    .line 1530
    .line 1531
    .line 1532
    .line 1533
    .line 1534
    .line 1535
    .line 1536
    .line 1537
    .line 1538
    .line 1539
    .line 1540
    .line 1541
    .line 1542
    .line 1543
    .line 1544
    .line 1545
    .line 1546
    .line 1547
    .line 1548
    .line 1549
    .line 1550
    .line 1551
    .line 1552
    .line 1553
    .line 1554
    .line 1555
    .line 1556
    .line 1557
    .line 1558
    .line 1559
    .line 1560
    .line 1561
    .line 1562
    .line 1563
    .line 1564
    .line 1565
    .line 1566
    .line 1567
    .line 1568
    .line 1569
    .line 1570
.end method

.method private printBuffer(Ljava/lang/String;[B)V
    .locals 0

    .line 1
    return-void
    .line 2
    .line 3
    .line 4
    .line 5
    .line 6
    .line 7
    .line 8
    .line 9
    .line 10
    .line 11
    .line 12
    .line 13
    .line 14
    .line 15
    .line 16
    .line 17
    .line 18
    .line 19
    .line 20
    .line 21
    .line 22
    .line 23
    .line 24
    .line 25
    .line 26
    .line 27
    .line 28
    .line 29
    .line 30
    .line 31
    .line 32
    .line 33
    .line 34
    .line 35
    .line 36
    .line 37
.end method

.method private readHRDParameters(Lcom/tencent/ugc/decoder/H264RbspRewriter;)V
    .locals 4
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 1
    .line 2
    const-string/jumbo v0, "SPS: cpb_cnt_minus1"

    .line 3
    .line 4
    .line 5
    invoke-virtual {p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Ljava/lang/String;)I

    .line 6
    move-result v0

    .line 7
    .line 8
    const-string/jumbo v1, "HRD: bit_rate_scale"

    .line 9
    const/4 v2, 0x4

    .line 10
    .line 11
    .line 12
    invoke-virtual {p1, v2, v1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 13
    .line 14
    const-string/jumbo v1, "HRD: cpb_size_scale"

    .line 15
    .line 16
    .line 17
    invoke-virtual {p1, v2, v1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 18
    const/4 v1, 0x0

    .line 19
    .line 20
    :goto_0
    if-gt v1, v0, :cond_0

    .line 21
    .line 22
    const-string/jumbo v2, "HRD: bit_rate_value_minus1"

    .line 23
    .line 24
    .line 25
    invoke-virtual {p1, v2}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 26
    .line 27
    const-string/jumbo v2, "HRD: cpb_size_value_minus1"

    .line 28
    .line 29
    .line 30
    invoke-virtual {p1, v2}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    .line 31
    .line 32
    const-string/jumbo v2, "HRD: cbr_flag"

    .line 33
    const/4 v3, 0x1

    .line 34
    .line 35
    .line 36
    invoke-virtual {p1, v3, v2}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 37
    .line 38
    add-int/lit8 v1, v1, 0x1

    .line 39
    goto :goto_0

    .line 40
    .line 41
    :cond_0
    const-string/jumbo v0, "HRD: initial_cpb_removal_delay_length_minus1"

    .line 42
    const/4 v1, 0x5

    .line 43
    .line 44
    .line 45
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 46
    .line 47
    const-string/jumbo v0, "HRD: cpb_removal_delay_length_minus1"

    .line 48
    .line 49
    .line 50
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 51
    .line 52
    const-string/jumbo v0, "HRD: dpb_output_delay_length_minus1"

    .line 53
    .line 54
    .line 55
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 56
    .line 57
    const-string/jumbo v0, "HRD: time_offset_length"

    .line 58
    .line 59
    .line 60
    invoke-virtual {p1, v1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    .line 61
    return-void
    .line 62
    .line 63
    .line 64
    .line 65
    .line 66
    .line 67
    .line 68
    .line 69
    .line 70
    .line 71
    .line 72
    .line 73
.end method


# virtual methods
.method public updateVUIforMaxBuffering(Ljava/io/InputStream;)[B
    .locals 7
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 8
    new-instance v0, Ljava/io/ByteArrayOutputStream;

    invoke-direct {v0}, Ljava/io/ByteArrayOutputStream;-><init>()V

    .line 9
    new-instance v1, Lcom/tencent/ugc/decoder/H264RbspRewriter;

    invoke-direct {v1, p1, v0}, Lcom/tencent/ugc/decoder/H264RbspRewriter;-><init>(Ljava/io/InputStream;Ljava/io/OutputStream;)V

    const-string/jumbo p1, "NALU type"

    const/16 v2, 0x8

    .line 10
    invoke-virtual {v1, v2, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo p1, "SPS: profile_idc"

    .line 11
    invoke-virtual {v1, v2, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readNBit(ILjava/lang/String;)J

    move-result-wide v3

    long-to-int p1, v3

    const-string/jumbo v3, "SPS: constraint_set_0-3_flag and reserved_zero_4bits"

    .line 12
    invoke-virtual {v1, v2, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo v3, "SPS: level_idc"

    .line 13
    invoke-virtual {v1, v2, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readNBit(ILjava/lang/String;)J

    const-string/jumbo v3, "SPS: seq_parameter_set_id"

    .line 14
    invoke-virtual {v1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const/16 v3, 0x64

    const/4 v4, 0x0

    const/4 v5, 0x1

    if-eq p1, v3, :cond_0

    const/16 v3, 0x6e

    if-eq p1, v3, :cond_0

    const/16 v3, 0x7a

    if-eq p1, v3, :cond_0

    const/16 v3, 0x90

    if-ne p1, v3, :cond_4

    :cond_0
    const-string/jumbo p1, "SPS: chroma_format_idc"

    .line 15
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Ljava/lang/String;)I

    move-result p1

    const/4 v3, 0x3

    if-ne p1, v3, :cond_1

    const-string/jumbo p1, "SPS: residual_color_transform_flag"

    .line 16
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    :cond_1
    const-string/jumbo p1, "SPS: bit_depth_luma_minus8"

    .line 17
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: bit_depth_chroma_minus8"

    .line 18
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: qpprime_y_zero_transform_bypass_flag"

    .line 19
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo p1, "SPS: seq_scaling_matrix_present_lag"

    .line 20
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    move-result p1

    if-eqz p1, :cond_4

    const/4 p1, 0x0

    :goto_0
    if-ge p1, v2, :cond_4

    const-string/jumbo v3, "SPS: seqScalingListPresentFlag"

    .line 21
    invoke-virtual {v1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    move-result v3

    if-eqz v3, :cond_3

    const/4 v3, 0x6

    if-ge p1, v3, :cond_2

    const/16 v3, 0x10

    .line 22
    invoke-virtual {v1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipScalingList(I)V

    goto :goto_1

    :cond_2
    const/16 v3, 0x40

    .line 23
    invoke-virtual {v1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipScalingList(I)V

    :cond_3
    :goto_1
    add-int/lit8 p1, p1, 0x1

    goto :goto_0

    :cond_4
    const-string/jumbo p1, "SPS: log2_max_frame_num_minus4"

    .line 24
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: pic_order_cnt_type"

    .line 25
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Ljava/lang/String;)I

    move-result p1

    if-nez p1, :cond_5

    const-string/jumbo p1, "SPS: log2_max_pic_order_cnt_lsb_minus4"

    .line 26
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    goto :goto_3

    :cond_5
    if-ne p1, v5, :cond_6

    const-string/jumbo p1, "SPS: delta_pic_order_always_zero_flag"

    .line 27
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo p1, "SPS: offset_for_non_ref_pic"

    .line 28
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: offset_for_top_to_bottom_field"

    .line 29
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: num_ref_frames_in_pic_order_cnt_cycle"

    .line 30
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Ljava/lang/String;)I

    move-result p1

    const/4 v2, 0x0

    :goto_2
    if-ge v2, p1, :cond_6

    .line 31
    new-instance v3, Ljava/lang/StringBuilder;

    const-string/jumbo v6, "SPS: offsetForRefFrame ["

    invoke-direct {v3, v6}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v3, v2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string/jumbo v6, "]"

    invoke-virtual {v3, v6}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-virtual {v1, v3}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    add-int/lit8 v2, v2, 0x1

    goto :goto_2

    :cond_6
    :goto_3
    const-string/jumbo p1, "SPS: num_ref_frames"

    .line 32
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readUE(Ljava/lang/String;)I

    const-string/jumbo p1, "SPS: gaps_in_frame_num_value_allowed_flag"

    .line 33
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo p1, "SPS: pic_width_in_mbs_minus1"

    .line 34
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: pic_height_in_map_units_minus1"

    .line 35
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: frame_mbs_only_flag"

    .line 36
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    move-result p1

    if-nez p1, :cond_7

    const-string/jumbo p1, "SPS: mb_adaptive_frame_field_flag"

    .line 37
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    :cond_7
    const-string/jumbo p1, "SPS: direct_8x8_inference_flag"

    .line 38
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipNBit(ILjava/lang/String;)V

    const-string/jumbo p1, "SPS: frame_cropping_flag"

    .line 39
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->readBool(Ljava/lang/String;)Z

    move-result p1

    if-eqz p1, :cond_8

    const-string/jumbo p1, "SPS: frame_crop_left_offset"

    .line 40
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: frame_crop_right_offset"

    .line 41
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: frame_crop_top_offset"

    .line 42
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    const-string/jumbo p1, "SPS: frame_crop_bottom_offset"

    .line 43
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->skipUE(Ljava/lang/String;)V

    :cond_8
    const-string/jumbo p1, "SPS: vui_parameters_present_flag"

    .line 44
    invoke-virtual {v1, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->testBool(Ljava/lang/String;)Z

    move-result p1

    const-string/jumbo v2, "VUI set 1: "

    if-eqz p1, :cond_9

    .line 45
    invoke-virtual {v1, v5, v2}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    .line 46
    invoke-direct {p0, v1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->modifyVUIParameters(Lcom/tencent/ugc/decoder/H264RbspRewriter;)Z

    move-result p1

    if-nez p1, :cond_a

    const/4 p1, 0x0

    return-object p1

    .line 47
    :cond_9
    invoke-virtual {v1, v5, v2}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: aspect_ratio_info_present_flag"

    .line 48
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: overscan_info_present_flag"

    .line 49
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: video_signal_type_present_flag"

    .line 50
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: chroma_loc_info_present_flag"

    .line 51
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: timing_info_present_flag"

    .line 52
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: nal_hrd_parameters_present_flag"

    .line 53
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: vcl_hrd_parameters_present_flag"

    .line 54
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: pic_struct_present_flag"

    .line 55
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: bitstream_restriction_flag"

    .line 56
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: motion_vectors_over_pic_boundaries_flag"

    .line 57
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeBool(ZLjava/lang/String;)V

    const-string/jumbo p1, "VUI: max_bytes_per_pic_denom"

    .line 58
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    const-string/jumbo p1, "VUI: max_bits_per_mb_denom"

    .line 59
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    const-string/jumbo p1, "VUI: log2_max_mv_length_horizontal"

    const/16 v2, 0xa

    .line 60
    invoke-virtual {v1, v2, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    const-string/jumbo p1, "VUI: log2_max_mv_length_vertical"

    .line 61
    invoke-virtual {v1, v2, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    const-string/jumbo p1, "VUI: num_reorder_frames"

    .line 62
    invoke-virtual {v1, v4, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    const-string/jumbo p1, "VUI: max_dec_frame_buffering"

    .line 63
    invoke-virtual {v1, v5, p1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeUE(ILjava/lang/String;)V

    .line 64
    iget-boolean p1, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    if-nez p1, :cond_a

    const-string/jumbo p1, "H264SPSModifier"

    const-string/jumbo v2, "decode: add max_dec_frame_buffering 1 when vui is no exist"

    .line 65
    invoke-static {p1, v2}, Lcom/tencent/liteav/base/util/LiteavLog;->w(Ljava/lang/String;Ljava/lang/String;)V

    .line 66
    iput-boolean v5, p0, Lcom/tencent/ugc/decoder/H264SPSModifier;->mReceiveFirstVUI:Z

    .line 67
    :cond_a
    invoke-virtual {v1}, Lcom/tencent/ugc/decoder/H264RbspRewriter;->writeTrailingBits()V

    .line 68
    invoke-virtual {v0}, Ljava/io/ByteArrayOutputStream;->toByteArray()[B

    move-result-object p1

    return-object p1
.end method

.method public updateVUIforMaxBuffering([B)[B
    .locals 2
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    const-string/jumbo v0, "origin sps : "

    .line 1
    invoke-direct {p0, v0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->printBuffer(Ljava/lang/String;[B)V

    .line 2
    invoke-direct {p0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->deEmulationPrevention([B)[B

    move-result-object v0

    if-eqz v0, :cond_0

    const-string/jumbo p1, "deEmulationPrevention sps : "

    .line 3
    invoke-direct {p0, p1, v0}, Lcom/tencent/ugc/decoder/H264SPSModifier;->printBuffer(Ljava/lang/String;[B)V

    const/4 p1, 0x1

    move-object p1, v0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    .line 4
    :goto_0
    new-instance v1, Ljava/io/ByteArrayInputStream;

    invoke-direct {v1, p1}, Ljava/io/ByteArrayInputStream;-><init>([B)V

    invoke-virtual {p0, v1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->updateVUIforMaxBuffering(Ljava/io/InputStream;)[B

    move-result-object p1

    const-string/jumbo v1, "new sps : "

    .line 5
    invoke-direct {p0, v1, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->printBuffer(Ljava/lang/String;[B)V

    if-eqz p1, :cond_1

    if-eqz v0, :cond_1

    .line 6
    invoke-direct {p0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->emulationPrevention([B)[B

    move-result-object p1

    const-string/jumbo v0, "emulationPrevention sps : "

    .line 7
    invoke-direct {p0, v0, p1}, Lcom/tencent/ugc/decoder/H264SPSModifier;->printBuffer(Ljava/lang/String;[B)V

    :cond_1
    return-object p1
.end method
